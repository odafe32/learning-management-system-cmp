...
>
                    ...
                    <div class="card-body p-0">
                        ...
                        <div class="border-bottom border-gray-100">
                            <nav class="nav nav-pills nav-fill">
                                <a class="nav-link {{ $filter === 'all' ? 'active' : '' }}" href="{{ route('admin.messages.index', ['filter' => 'all']) }}">
                                    All
                                </a>
                                <a class="nav-link {{ $filter === 'unread' ? 'active' : '' }}" href="{{ route('admin.messages.index', ['filter' => 'unread']) }}">
                                    Unread
                                    @if($stats['unread_count'] > 0)
                                        <span class="badge bg-warning text-white ms-2">{{ $stats['unread_count'] }}</span>
                                    @endif
                                </a>
                                <a class="nav-link {{ $filter === 'sent' ? 'active' : '' }}" href="{{ route('admin.messages.index', ['filter' => 'sent']) }}">
                                    Sent
                                </a>
                            </nav>
                        </div>
                            ...
                            <div class="d-flex align-items-center justify-content-between">
                                <div class="d-flex align-items-center gap-12">
                                    ...
                                    <div>
                                        ...
                                        @if($conversationPartner->department)
                                            <span class="text-sm text-secondary-light"> • {{ $conversationPartner->department }}</span>
                                        @endif
                                    </div>
                                </div>
                                <div class="d-flex align-items-center gap-8">
                                    ...
                                    <a href="{{ route('admin.messages.index') }}" class="btn btn-gray radius-8 px-16 py-8">
                    ...
                    <form id="composeForm" enctype="multipart/form-data">
                        ...
                        <div class="mb-20">
                            ...
                            <select name="receiver_id" id="recipient" class="form-select radius-8" required>
                                ...
                                <optgroup label="Instructors & Lecturers">
                                    @foreach($availableUsers->whereIn('role', ['instructor', 'lecturer']) as $user)
                                        <option value="{{ $user->id }}" data-role="{{ $user->role }}" data-avatar="{{ $user->profile_image_url ?? '/assets/images/default-avatar.png' }}">
                                            {{ $user->name }} ({{ $user->email }})
                                        </option>
                                    @endforeach
                                </optgroup>
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            ...
            document.querySelectorAll('.conversation-item').forEach(item => {
                item.addEventListener('click', function() {
                    ...
                    window.location.href = `{{ route('admin.messages.index') }}?conversation=${userId}`;
                });
            });
        ...
        function sendMessage(form, isCompose = false) {
            ...
            const url = '{{ route("admin.messages.send") }}';
            ...
            .then(data => {
                ...
                if (data.success) {
                    if (isCompose) {
                        ...
                        setTimeout(() => {
                            window.location.href = `{{ route('admin.messages.index') }}?conversation=${formData.get('receiver_id')}`;
                        }, 1000);
                    } else {
        ...
        function getFileIconJS(extension) {
            const iconMap = {
                ...
                'mp3': 'music-note', 'wav': 'music-note', 'mp4': 'video', 'avi': 'video',
        ...
        function getFileTypeJS(extension) {
            const typeMap = {
                ...
                'txt': 'Text File', 'mp3': 'Audio File', 'mp4': 'Video File',
        ...
        // Reset compose modal when closed
